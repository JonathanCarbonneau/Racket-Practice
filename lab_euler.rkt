...
(define (get-vertices G)
  (usort (helperVertices G)))
(define (helperVertices G)
  (cond ((null? G) '())
      ((null? (cdr G)) (list (car (car G)) (car (cdr (car G)))))
      (else (append (list (car (car G)) (car (cdr (car G)))) (helperVertices (cdr G))))
      ))
...
(define (degreeHelper G v c)
  (cond
    ((null? G) c)
    ((and (equal? v (car(cdr(car G)))) (equal? v (car (car G)))) (degreeHelper (cdr G) v (+ 2 c)) )
    ((equal? v (car(cdr(car G)))) (degreeHelper (cdr G) v (+ 1 c)) )
    ((equal? v (car (car G))) (degreeHelper (cdr G) v (+ 1 c)))
    (else (degreeHelper (cdr G) v c))
    ))
...
(define (all-evenHelper G lst)
  (cond
       ((null? lst) #t)
       ((null? (cdr lst)) (even? (degree G (car lst))))
       (else (and (even? (degree G (car lst))) (all-evenHelper G (cdr lst))))
       ))
...
(define (adjacentHelper G v)
  (if (null? G) '()
  (if (null? (cdr G))
  (if (equal? v (car (car G))) (list (car (cdr (car G))))
  (if (equal? v (car (cdr (car G)))) (list (car (car G))) '()))
  (if (equal? v (car (car G))) (append (adjacentHelper (cdr G) v) (list (car (cdr (car G)))))
  (if (equal? v (car (cdr (car G)))) (append (adjacentHelper (cdr G) v) (list (car (car G)))) (adjacentHelper (cdr G) v))))))
...
(define (connected? G)
  (if (null? G) #t (connectHelper (get-vertices G) (dfs G (car (car G))))))

(define (connectHelper lst lst2)
 ( cond
  ((null? lst) #t)
  ((null? (cdr lst)))
  (else (and (member (car lst) lst2) (connectHelper (cdr lst) lst2)))))
