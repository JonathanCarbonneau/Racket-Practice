...
(define (mod-exp b e m)
  (cond
    ...
    ((odd? e)  (modulo (* (modulo b m) (modulo (mod-exp b (- e 1) m)m)) m))
    ))
...
(define (encrypt plaintext key)
  (encode (genList (string->list plaintext)) (car key) (car (cdr key))))
(define (genList l)
  (cond ((null? (cdr l)) (list (char->integer (car l)))) (else (append (list (char->integer (car l))) (genList (cdr l))))))
(define (encode l m e)
  (cond ((null? (cdr l)) (list (mod-exp (car l) e m))) (else(append (list (mod-exp (car l) e m)) (encode (cdr l) m e)))))
...
(define (decrypt ciphertext key)
  (list->string (helper1 (helper ciphertext (car key) (car (cdr key))))))
(define (helper l m d)
  (cond ((null? (cdr l)) (list (mod-exp (car l) d m))) (else (append (list (mod-exp (car l) d m)) (helper (cdr l) m d)))))
(define (helper1 l)
   (cond ((null? (cdr l)) (list (integer->char (car l)))) (else (append (list (integer->char (car l))) (helper1 (cdr l))))))
